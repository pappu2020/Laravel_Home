...
class bloggerWebsiteController extends Controller
{
    function bloggerWebsitePage()
    {
        ...
        // $allPopularpost = likeDislikeModel::groupBy('post')
        //     ->selectRaw('sum(like) as sum, post')
        //     ->havingRaw('sum >= 1')
        //     ->take(3)
        //     ->orderBy("sum", "DESC")
        //     ->get();
        ...
        $allPopularpost = likeDislikeModel::select("post")
            ->groupBy('post')
            ->selectRaw('post, count(post) as sum')
            ->havingRaw('sum >= 1')
            ->get();
